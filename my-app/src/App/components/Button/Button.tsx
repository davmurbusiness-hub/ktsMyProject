...
const Button: React.FC<ButtonProps> = ({
    ...
    return (
        <button className={cn(className, s.button, disabled && s.button_disabled, outline && s.outline)}
            ...
            <Text view={"button"}
                  weight={'normal'}
                  className={cn(s[`text-button${outline && '-outline'}`],
                      isHovered && s['text-button-white'],
                      isActive && s['text-button-white'])}>{children}</Text>
        </button>
    )
};
